...
var Pinia = (function (exports, vueDemi) {
      ...
      setupDevtoolsPlugin({
          ...
          store.$dispose = () => {
              ...
              api.getSettings().logStoreChanges &&
                  toastMessage(`Disposed "${store.$id}" store ð`);
          };
          ...
          api.sendInspectorState(INSPECTOR_ID);
          api.getSettings().logStoreChanges &&
              toastMessage(`"${store.$id}" store installed ð`);
      });
  }
      ...
      function setup() {
          ...
          return assign(localState, actions, Object.keys(getters || {}).reduce((computedGetters, name) => {
              if (name in localState) {
                  console.warn(`[ð]: A getter cannot have the same name as another state property. Rename one of them. Found with "${name}" in store "${id}".`);
              }
  ...
  function createSetupStore($id, setup, options = {}, pinia, hot, isOptionsStore) {
          ...
          $subscribeOptions.onTrigger = (event) => {
              ...
              if (isListening) {
                  ...
                  if (Array.isArray(debuggerEvents)) {
                      ...
                      console.error('ð debuggerEvents should be an array. This is most likely an internal Pinia bug.');
                  }
              }
          };
      }
      ...
      const $reset = isOptionsStore
          ...
          : /* istanbul ignore next */
              () => {
                      throw new Error(`ð: Store "${$id}" is built using the setup syntax and does not implement $reset().`);
                  }
                  ;
      function $dispose() {
          scope.stop();
      ...
      if (store.$state &&
          ...
          console.warn(`[ð]: The "state" must be a plain object. It cannot be\n` +
              `\tstate: () => new MyClass()\n` +
  ...
  idOrOptions, setup, setupOptions) {
      ...
      if (typeof idOrOptions === 'string') {
          ...
          id = idOrOptions.id;
          if (typeof id !== 'string') {
              throw new Error(`[ð]: "defineStore()" must be passed a store id as its first argument.`);
          }
      }
      function useStore(pinia, hot) {
          ...
          if (pinia)
              setActivePinia(pinia);
          if (!activePinia) {
              throw new Error(`[ð]: "getActivePinia()" was called but there was no active Pinia. Are you trying to use a store before calling "app.use(pinia)"\n` +
  ...
  function mapStores(...stores) {
      if (Array.isArray(stores[0])) {
          console.warn(`[ð]: Directly pass all stores to "mapStores()" without putting them in an array:\n` +
              `Replace\n` +
