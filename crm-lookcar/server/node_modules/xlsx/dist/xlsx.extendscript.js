...
function pad0(v,d){var t=""+v; return t.length>=dt:fill('0',d-t.length)+t;}
function pad_(v,d){var t=""+v;return t.length>=dt:fill(' ',d-t.length)+t;}
function rpad_(v,d){var t=""+v; return t.length>=dt:t+fill(' ',d-t.length);}
function pad0r1(v,d){var t=""+Math.round(v); return t.length>=dt:fill('0',d-t.length)+t;}
function pad0r2(v,d){var t=""+v; return t.length>=dt:fill('0',d-t.length)+t;}
...
function SSF_large_exp(v) {
	...
	return (o.length > (v<012:11) || o === "0" || o === "-0")  v.toPrecision(6) : o;
}
...
function write_num_flt(type, fmt, val) {
	...
	if((r = fmt.match(/^([0#]+)(\\-([0#]+))+$/))) {
		...
		ri = 0;
		return _strrev(_strrev(fmt.replace(/\\/g,"")).replace(/[0#]/g,function(x){return ri<o.lengtho.charAt(ri++):x==='0''0':"";}));
	}
	if(fmt.match(phone)) {
...
out[i].v = SSF_write_date(out[i].t.charCodeAt(0), out[i].v, dt, ss0);
	...
	if(nstr.length > 0) {
		...
		if(decpt === out.length && ostr.indexOf("E") === -1) {
			...
			for(i=decpt; i>= 0; --i) {
				if(out[i] == null || 'n'.indexOf(out[i].t) === -1) continue;
				j=out[i].v.indexOf(".")>-1&&i===decptout[i].v.indexOf(".")-1:out[i].v.length-1;
			...
			for(i=decpt; i<out.length; ++i) {
				...
				j=out[i].v.indexOf(".")>-1&&i===decptout[i].v.indexOf(".")+1:0;
...
function choose_fmt(f, v) {
	...
	if(typeof v !== "number") return [4, fmt.length === 4 || lat>-1fmt[fmt.length-1]:"@"];
	switch(fmt.length) {
...
function dateNF_fix(str, dateNF, match) {
	...
	var datestr = (("" + (Y>=0Y: new Date().getFullYear())).slice(-4) + "-" + ("00" + (m>=1m:1)).slice(-2) + "-" + ("00" + (d>=1d:1)).slice(-2));
	if(datestr.length == 7) datestr = "0" + datestr;
	if(datestr.length == 8) datestr = "20" + datestr;
	var timestr = (("00" + (H>=0H:0)).slice(-2) + ":" + ("00" + (M>=0M:0)).slice(-2) + ":" + ("00" + (S>=0S:0)).slice(-2));
...
function read_directory(dir_start, sector_list, sectors, Paths, nmfs, files, FileIndex, mini) {
	var minifat_store = 0, pl = (Paths.length2:0);
...
var unescapexml = (function() {
	...
	return function unescapexml(text) {
		...
		if(i == -1) return s.replace(encregex, function($$, $1) { return encodings[$$]||String.fromCharCode(parseInt($1,$$.indexOf("x")>-116:10))||$$; }).replace(coderegex,function(m,c) {return String.fromCharCode(parseInt(c,16));});
		var j = s.indexOf("]]>");
...
function parse_URLMoniker(blob) {
	...
	var url = blob.read_shift((extralen-24:len)>>1, 'utf16le').replace(chr0,"");
	if(extra) blob.l += 24;
	return url;
}
...
function rgb2Hex(rgb) {
	for(var i=0,o=1; i!=3; ++i) o = o*256 + (rgb[i]>255255:rgb[i]<00:rgb[i]);
...
var rc_to_a1 = (function(){
	...
	function rcfunc($$,$1,$2,$3) {
		...
		var R = $2.length>0parseInt($2,10)|0:0, C = $3.length>0parseInt($3,10)|0:0;
